  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-blue-50">
      <div className="text-center px-4">
        ...
        <p className="text-xl text-gray-600 mb-8">Autor no encontrado</p>
        <a 
          href="/descubre"
...
export default function AuthorPage() {
  ...
  const { data, loading, error } = useAuthor(slug)

  useEffect(() => {
    if (data?.seo) {
      document.title = data.seo.title || `${data.name} - Atlas Vivo`
    ...
    <>
      ...
      <main>
        <DetailHero 
          title={data.name}
          subtitle={data.role}
          year={`${data.birthYear} - ${data.deathYear || 'actualidad'}`}
          image={data.image}
          backTo="/descubre"
